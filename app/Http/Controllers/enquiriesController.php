...
class enquiriesController extends Controller {
	...
	public function listAll($page = 1,$search = ""){
		...
		$toReturn["enquiries"] = $toReturn["enquiries"]->select('id','enq_title','enq_type','enq_source','FullName','phoneNo','email','Address','enq_date','nxt_fup','enq_file')->take('20')->skip(20* ($page - 1) )->get()->toArray();
	...
	public function create(){
		...
		if (\Input::hasFile('enq_file')) {
			...
			$fileInstance->move('uploads/enquiries',$newFileName);
	...
	public function edit($id){
		...
		if (\Input::hasFile('enq_file')) {
			...
			if($enquiries->enq_file != ""){
				@unlink("uploads/enquiries".$enquiries->enq_file);
			}
	...
	public function delete($id){
		...
		if ( $postDelete = \enquiries::where('id', $id)->first() ){
        		if($postDelete->enq_file != ""){ @unlink('uploads/enquiries'.$postDelete->enq_file); }
