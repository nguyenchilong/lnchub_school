...
class rolesController extends Controller {
	...
	public function listAll(){
		...
		$toReturn["roles_perms"] = array();

		foreach ($this->roles_perms as $key => $value) {
			foreach ($value as $key_ => $value_) {
				if(isset($this->panelInit->language[$value_])){
					$toReturn["roles_perms"][$key][$value_] = $this->panelInit->language[$value_];
				}else{
					$toReturn["roles_perms"][$key][$value_] = $value_;
				}
			}
		}

		//Add permissions for messages section
		foreach ($toReturn["roles"] as $key => $value) {
			$toReturn["roles_perms"]['Messages']['r_'.$value['id']] = $this->panelInit->language['canMessage']." ".$value['role_title'];
		}
	...
	public function check_exist_perm($perm_to_check){
		if( !is_array($this->roles_perms_combined) ){
			...
			$roles = \roles::select('id')->get();
			foreach ($roles as $key => $value) {
				$this->roles_perms_combined[] = 'Messages.r_'.$value->id;
			}
		}
