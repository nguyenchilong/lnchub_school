...
class SiteSettingsController extends Controller {
	...
	public function listAll($title)
	{
		if($title == "siteSettings"){
			...
			$settingsArray['settings']['absentNotifWhen'] = json_decode($settingsArray['settings']['absentNotifWhen'],true);
			if(!is_array($settingsArray['settings']['absentNotifWhen'])){
				$settingsArray['settings']['absentNotifWhen'] = array();
			}
			...
			if(isset($settingsArray['settings']['zoomApiSecret'])){
				unset($settingsArray['settings']['zoomApiSecret']);
			}
	...
	public function save($title){
		if($title == "siteSettings"){
			...
			if(isset($settingsData['zoomApiKey']) AND isset($settingsData['zoomApiSecret'])){

				$zoom = new \zoom_integration($settingsData['zoomApiKey'],$settingsData['zoomApiSecret']);
				$response = $zoom->get_user(); 
				if ( $response === false OR !isset($response['users']) OR count($response['users']) == 0  ) {
					return $this->panelInit->apiOutput(false,$this->panelInit->language['editSettings'],'Zoom API details are invalid, Please validate & save');
				}
