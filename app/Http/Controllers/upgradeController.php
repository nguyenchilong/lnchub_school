...
class upgradeController extends Controller {
		...
		try{
			...
			if(\Schema::hasTable('settings')){
				...
				if($testInstalled->count() > 0){
					...
					if($testInstalled->fieldValue >= $this->version){
	...
	public function proceed()
	{
		if(\Input::get('nextStep') == "1"){
			...
			if(!isset($loginError)) {
				...
				@file_put_contents("uploads/cards/test", $testData);
				...
				if(@file_get_contents("uploads/cards/test") != $testData){
					$this->data['perrors'][] = "uploads/cards";
					$this->data['nextStep'] = "1";
				}else{
					$this->data['success'][] = "uploads/cards";
				}	
		...
		if(\Input::get('nextStep') == "2"){
			...
			if($testInstalled->fieldValue == "5.1"){
				\DB::unprepared(file_get_contents('storage/app/dbsqlUp60'));

				$settings = \settings::where('fieldName','thisVersion')->first();
				$settings->fieldValue = '6.0';
				$settings->save();


				$testInstalled->fieldValue = "6.0";
			}

			if($testInstalled->fieldValue == "6.0"){
				\DB::unprepared(file_get_contents('storage/app/dbsqlUp70'));

				$settings = \settings::where('fieldName','thisVersion')->first();
				$settings->fieldValue = '7.0';
				$settings->save();


				$testInstalled->fieldValue = "7.0";
			}

			$this->upgrade_languages();
		}
	...
	public function upgrade_languages(){

		$lang_array = array();
		$current_langs = \languages::get();
		foreach ($current_langs as $key => $value) {
			$lang_array[$value->id] = json_decode($value->languagePhrases,true);
		}

		$lang_json = file_get_contents("storage/app/dbsqlLang");
		$lang_json = json_decode($lang_json,true);

		foreach ($lang_json as $lang_key => $phrases) {
			if(!isset($lang_array[$lang_key])){
				continue;
			}
			
			$phrases['languagePhrases'] = json_decode($phrases['languagePhrases'],true);
			foreach ($phrases['languagePhrases'] as $key => $value) {
				if(!isset($lang_array[$lang_key][$key])){
					$lang_array[$lang_key][$key] = $value;
				}
			}

			if( count($lang_array[$lang_key]) > count($phrases['languagePhrases']) ){
				foreach ($lang_array[$lang_key] as $key => $value) {
					if(!isset($phrases['languagePhrases'][$key])){
						unset($lang_array[$lang_key][$key]);
					}
				}
			}

			\languages::where('id',$lang_key)->update( array( "languagePhrases"=>json_encode($lang_array[$lang_key]) ) );
		}
